    ...
    <div class="bg-card-warm rounded-3xl p-8 border border-dark-charcoal/10 shadow-xl orange-glow">
      ...
      <form onsubmit={handleLogin} class="space-y-5">
        ...
        <div class="flex items-center justify-between text-sm">
          <label class="flex items-center gap-2 font-semibold text-dark-charcoal/70 cursor-pointer">
            <input
              ...
              class="rounded border-dark-charcoal/20 text-accent-orange focus:ring-accent-orange w-4 h-4 accent-accent-orange"
            />
            Remember Me
          </label>
          
          <button type="button" onclick={() => alert('Demo account reset: Use one of the quick autofill credentials below.')} class="font-bold text-accent-orange hover:underline cursor-pointer">
            Forgot Password?
          </button>
        </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          class="w-full bg-accent-orange text-bg-warm font-bold py-3.5 rounded-xl hover:bg-dark-charcoal transition-all duration-300 shadow-md orange-glow cursor-pointer disabled:opacity-50"
        >
          {#if isLoading}
      ...
      <div class="mt-4 text-center text-xs font-bold text-dark-charcoal/50">
        ...
        <a href="/register" class="text-accent-orange hover:underline ml-1">Sign Up</a>
      </div>
      ...
      <div class="mt-8 border-t border-dark-charcoal/10 pt-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          <button
            ...
            class="bg-bg-warm border border-dark-charcoal/10 hover:border-accent-orange/40 rounded-xl p-2.5 text-center cursor-pointer transition-colors"
          >
